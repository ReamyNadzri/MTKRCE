...
class Config:
    ...
    # --- Model Path ---
    MODEL_PATH = 'kuih_recognition_model_resnet50.keras' 
    TARGET_SIZE = (224, 224)
...
# --- UPDATED: predict_kuih for ResNet50 ---
def predict_kuih(image_path):
    ...
    try:
        ...
        # Use ResNet50's preprocess_input
...
@app.route('/')
def home():
    return render_template('index_resnet50.html',
...
def handle_predict():
    ...
    if not model_loaded:
        return render_template('index_resnet50.html', error="Model not loaded.", **render_args), 503

    file = request.files.get('file')
    if not file or file.filename == '':
        return render_template('index_resnet50.html', error="No file selected.", **render_args), 400
    if not allowed_file(file.filename):
        return render_template('index_resnet50.html', error="Invalid file type.", **render_args), 400
    ...
    try:
        ...
        if "Error" in kuih_name:
             ...
             return render_template('index_resnet50.html', error=kuih_name, **render_args), 500
        ...
        return render_template('index_resnet50.html', **render_args)

    except Exception as e:
        logger.error(f"Predict route error: {e}")
        return render_template('index_resnet50.html', error="Server error during prediction.", **render_args), 500
