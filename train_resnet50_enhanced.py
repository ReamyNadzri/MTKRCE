...
# Model Parameters
IMG_SIZE = (224, 224) # ResNet50 typically uses 224x224
BATCH_SIZE = 32
LEARNING_RATE = 0.001
MODEL_SAVE_PATH = 'kuih_recognition_model_resnet50_enhanced.keras'
EPOCHS_TO_TRAIN = 50
...
def main():
    ...
    # Training generator with data augmentation (Matched to ResNet50 notebook)
    ...
    # Validation/Test generator (only ResNet50 preprocessing)
    ...
    early_stop = EarlyStopping(
        monitor='val_loss', # Changed to val_loss based on notebook logic
        patience=5,
    ...
    logger.info("Saved training history plot to 'training_history_resnet50.png'")
    ...
    with open('evaluation_results_resnet50.txt', 'w') as f:
        f.write(f"Test Accuracy: {test_acc:.4f}\n")
        f.write(f"Test Loss: {test_loss:.4f}\n\n")
        f.write("--- Classification Report ---\n")
        f.write(report)
    logger.info("Saved evaluation results to 'evaluation_results_resnet50.txt'")
    ...
    plt.tight_layout()
    plt.savefig('confusion_matrix_resnet50_enhanced.png')
    logger.info("Saved confusion matrix to 'confusion_matrix_resnet50_enhanced.png'")
