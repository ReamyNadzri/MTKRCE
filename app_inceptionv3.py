...
class Config:
    ...
    # --- Model Path ---
    MODEL_PATH = 'kuih_recognition_model_inceptionv3.keras' 
    # --- UPDATED: Target Size ---
    TARGET_SIZE = (299, 299)
    MIN_CONFIDENCE_THRESHOLD = 0.7

# --- Flask App Setup ---
app = Flask(__name__, template_folder='.') # Tell Flask to look for templates in the current folder
...
# --- UPDATED: predict_kuih for InceptionV3 ---
def predict_kuih(image_path):
    ...
    try:
        ...
        # Use InceptionV3's preprocess_input
        img_processed = preprocess_input(img_arr_expanded) 
...
@app.route('/')
def home():
    return render_template('index_inceptionv3.html',
...
def handle_predict():
    ...
    if not model_loaded:
        return render_template('index_inceptionv3.html', error="Model not loaded.", **render_args), 503

    file = request.files.get('file')
    if not file or file.filename == '':
        return render_template('index_inceptionv3.html', error="No file selected.", **render_args), 400
    if not allowed_file(file.filename):
        return render_template('index_inceptionv3.html', error="Invalid file type.", **render_args), 400
    ...
    try:
        ...
        if "Error" in kuih_name:
             ...
             return render_template('index_inceptionv3.html', error=kuih_name, **render_args), 500
        ...
        return render_template('index_inceptionv3.html', **render_args)

    except Exception as e:
        logger.error(f"Predict route error: {e}")
        return render_template('index_inceptionv3.html', error="Server error during prediction.", **render_args), 500
